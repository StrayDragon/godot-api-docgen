...
/// <para><b>Note:</b> By default, this event is only emitted once per frame rendered at most. If you need more precise input reporting, set <see cref="Godot.Input.UseAccumulatedInput"/> to <see langword="false"/> to make events emitted as often as possible. If you use InputEventMouseMotion to draw lines, consider using <see cref="Godot.Geometry2D.BresenhamLine(Vector2I, Vector2I)"/> as well to avoid visible gaps in lines if the user is moving the mouse quickly.</para>
/// <para><b>Note:</b> This event may be emitted even when the mouse hasn't moved, either by the operating system or by Godot itself. If you really need to know if the mouse has moved (e.g. to suppress displaying a tooltip), you should check that <c>relative.is_zero_approx()</c> is <see langword="false"/>.</para>
/// </summary>
public partial class InputEventMouseMotion : InputEventMouse
{
    ...
    /// <para><b>Note:</b> Since <see cref="Godot.InputEventMouseMotion"/> may only be emitted when the mouse moves, it is not possible to reliably detect when the mouse has stopped moving by checking this property. A separate, short timer may be necessary.</para>
    ...
    /// <para><b>Note:</b> <see cref="Godot.InputEventMouseMotion.Velocity"/> is automatically scaled according to the content scale factor, which is defined by the project's stretch mode settings. That means mouse sensitivity may appear different depending on resolution.</para>
    /// <para><b>Note:</b> Use <see cref="Godot.InputEventMouseMotion.ScreenRelative"/> for mouse aiming using the <see cref="Godot.Input.MouseModeEnum.Captured"/> mouse mode.</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>The unscaled mouse velocity in pixels per second in screen coordinates. This velocity is <i>not</i> scaled according to the content scale factor or calls to <see cref="Godot.InputEvent.XformedBy(Transform2D, Nullable{Vector2})"/>.</para>
    /// <para><b>Note:</b> Use <see cref="Godot.InputEventMouseMotion.ScreenRelative"/> for mouse aiming using the <see cref="Godot.Input.MouseModeEnum.Captured"/> mouse mode.</para>
    /// </summary>
    ...
    internal InputEventMouseMotion(IntPtr ptr) : this(true)
    {
        NativePtr = ptr;
        unsafe
        {
    ...
    {
        NativeCalls.godot_icall_1_36(MethodBind0, GodotObject.GetPtr(this), &tilt);
    }
    ...
    {
        NativeCalls.godot_icall_1_67(MethodBind2, GodotObject.GetPtr(this), pressure);
    }
    ...
    {
        NativeCalls.godot_icall_1_14(MethodBind4, GodotObject.GetPtr(this), penInverted.ToGodotBool());
    }
    ...
    {
        NativeCalls.godot_icall_1_36(MethodBind6, GodotObject.GetPtr(this), &relative);
    }
    ...
    {
        NativeCalls.godot_icall_1_36(MethodBind8, GodotObject.GetPtr(this), &relative);
    }
    ...
    {
        NativeCalls.godot_icall_1_36(MethodBind10, GodotObject.GetPtr(this), &velocity);
    }
    ...
    {
        NativeCalls.godot_icall_1_36(MethodBind12, GodotObject.GetPtr(this), &velocity);
    }
